...
const DropdownExampleSearchSelection = (props) => {
  let countryOptions2=countryOptions.map((item)=>{
    return{
  ...
  const change=(e)=>{
    if (e.target.value !== undefined) {
      ...
      props.country(countryOptions2.filter((item) =>item.text.includes(e.target.value))[0].text)

      
    } 
    else {
     
        props.country(countryOptions2.filter((item) =>item.text.includes(e.target.textContent))[0].text)
    }
  }
  return(  <Dropdown
